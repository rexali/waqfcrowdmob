...
import { getTotalWaqfDonation, getWaqfDonations } from "./donationsSlice";

export default function ViewWaqfDonation() {
    ...
    const renderItem = (item: any) => {
        ...
        const waqf = waqfs.find((waqf: { waqfId: any }) => waqf.waqfId === item.waqfId);
        ...
        return <View style={styles.row}>
            <Text
                ...
                onPress={() => router.push({
                    ...
                    params: { waqfId: waqf.waqfId }
                })}
                style={{ ...styles.cell, color: "blue" }}>{item.name}</Text>
            <Text style={{ ...styles.cell }}>{item.amount}</Text>
            <Text style={styles.cell}>{item.category}</Text>
            <Text style={{ ...styles.cell, fontSize: 13, textAlign:"center"}}>
    ...
    return (
        <View style={styles.table}>
            <Stack.Screen options={{ title: "Waqf Donations"}}/>
